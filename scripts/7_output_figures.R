...
# ============================================================
# 4) PANEL 2 — Protein-to-energy inadequacy (EAR) relative map — 3-class
#   Robust to near-zero energy inadequacy
# ============================================================

# --- 3-class palette (same spirit as Script 6) ---
ratio_palette <- c(
...
map_ratio3_df <- map_EAR_df %>%
  mutate(
    # avoid divide-by-zero / unstable ratios when energy inadequacy is near-zero:
...
p_map_ratio3 <- ggplot(map_ratio3_df) +
  ...
  common_coord_sf() +
  labs(
    title = "Protein inadequacy (EAR) relative to energy inadequacy",
    fill  = NULL
  ) +
  tight_map_theme() +
  theme(
...
save_map_output(
  p_map_ratio3,
  "Fig3_panel_ratio_EAR_to_energy_3class",
  width = 12,
  height = 6.6
)
...
# ============================================================
# PANEL (new) — Mean protein intake (g/day), medium-calorie scenario
#   Country mean of protein_grams from scen3_long (calorie_scenario == "medium")
#   Uses SAME color scale style as Panel d (viridis magma, direction = -1)
# ============================================================
...
# 1) Country-level mean protein (g/day), population-weighted across sex × age
country_protein_g_medium <- scen3_long %>%
  filter(calorie_scenario == "medium") %>%
  group_by(iso3) %>%
  summarise(
...
# 2) Join to geometry
map_protein_g_df <- world %>%
  left_join(country_protein_g_medium, by = "parent_iso3")

# 3) Plot (same palette “feel” as protein-quality panel)
p_map_protein_g <- ggplot(map_protein_g_df) +
  geom_sf(aes(fill = protein_g_day), color = NA) +
  scale_fill_viridis_c(
    option = "magma",
    direction = -1,
    limits = c(0, max(map_protein_g_df$protein_g_day, na.rm = TRUE)),
    oob = scales::squish,
    na.value = na_gray,
  ...
  common_coord_sf() +
  labs(
    title = "Mean protein intake (g/day), medium-calorie scenario",
    fill  = "g/day"
  ) +
  tight_map_theme() +
  theme(legend.position = "bottom")
...
save_map_output(
  p_map_protein_g,
  "Fig3_panel_mean_protein_g_day_medium_scenario",
  width = 12,
  height = 6.6
)
...
# pB <- p_map_ratio3  + labs(tag = "b") + theme(plot.title = element_text(size = 13, face = "bold"))
# pC <- p_map_protein_g + labs(tag = "c") + theme(plot.title = element_text(size = 13, face = "bold"))
...
# ==============================================================
# High-burden strata: protein inadequacy (EAR) >= 25%
# Medium ("realistic") calorie scenario
# ==============================================================


# medium scenario only
med <- scen3_long %>%
  filter(calorie_scenario == "medium") %>%
  transmute(iso3, region_WB, sex, age_group, population, prot_inad_EAR) %>%
  filter(is.finite(prot_inad_EAR))



threshold <- 0.25

high <- med %>% filter(prot_inad_EAR >= threshold)



share_pop_high_by_region <- high %>%
  group_by(region_WB) %>%
  summarise(pop = sum(population, na.rm = TRUE), .groups = "drop") %>%
  mutate(share_within_high = pop / sum(pop)) %>%
  arrange(desc(pop))

share_pop_high_by_sex <- high %>%
  group_by(sex) %>%
  summarise(pop = sum(population, na.rm = TRUE), .groups = "drop") %>%
  mutate(share_within_high = pop / sum(pop)) %>%
  arrange(desc(pop))
...
global_share_pop_high <- sum(high$population, na.rm = TRUE) / sum(med$population, na.rm = TRUE)

cat("\nGlobal share of population in strata >=25%:",
    round(100*global_share_pop_high, 2), "%\n")
print(share_pop_high_by_region, n = nrow(share_pop_high_by_region))
print(share_pop_high_by_sex, n = nrow(share_pop_high_by_sex))


# ==============================================================
# Age distribution of population living in >=25% strata
# ==============================================================



# Broader age groups (cleaner narrative)
share_pop_high_by_age_broad <- high %>%
  mutate(age_start = suppressWarnings(as.numeric(sub("^([0-9]+).*", "\\1", age_group))),
         age_broad = case_when(
           age_start < 15 ~ "Children (1–14)",
           age_start < 50 ~ "Adults (15–49)",
           age_start < 65 ~ "Older adults (50–64)",
           TRUE ~ "Elderly (65+)"
         )) %>%
  group_by(age_broad) %>%
  summarise(pop = sum(population, na.rm = TRUE), .groups = "drop") %>%
  mutate(
    share_within_high = pop / sum(pop),
    share_pct = round(100 * share_within_high, 1)
  ) %>%
  arrange(desc(pop))

print(share_pop_high_by_age_broad)


# ==============================================================
# Distribution of globally inadequate population (EAR, medium)
# ==============================================================

med_all <- scen3_long %>%
  filter(calorie_scenario == "medium") %>%
  transmute(
    iso3, region_WB, sex, age_group,
    population,
    prot_inad_EAR
  ) %>%
  filter(is.finite(prot_inad_EAR)) %>%
  mutate(
    inadequate_pop = prot_inad_EAR * population
  )

# Total inadequate population globally
total_inadequate_pop <- sum(med_all$inadequate_pop, na.rm = TRUE)

cat("\nTotal inadequate population (absolute):", total_inadequate_pop, "\n")


share_inadequate_by_region <- med_all %>%
  group_by(region_WB) %>%
  summarise(
    inadequate_pop = sum(inadequate_pop, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
    share_of_global_inadequate = inadequate_pop / total_inadequate_pop,
    share_pct = round(100 * share_of_global_inadequate, 1)
  ) %>%
  arrange(desc(share_pct))

print(share_inadequate_by_region, n = nrow(share_inadequate_by_region))

share_inadequate_by_sex <- med_all %>%
  group_by(sex) %>%
  summarise(
    inadequate_pop = sum(inadequate_pop, na.rm = TRUE),
    .groups = "drop"
...
print(share_inadequate_by_sex)


# ==============================================================
# Age distribution of globally inadequate population (EAR, medium)
# ==============================================================

share_inadequate_by_age_broad <- med_all %>%
  mutate(
    age_start = suppressWarnings(as.numeric(sub("^([0-9]+).*", "\\1", age_group))),
    age_broad = case_when(
      age_start < 15 ~ "Children (1–14)",
...
print(share_inadequate_by_age_broad)



# ==============================================================
# Share of global population aged 65+ (medium scenario population)
# ==============================================================

global_age_distribution <- med_all %>%
  mutate(
    age_start = suppressWarnings(as.numeric(sub("^([0-9]+).*", "\\1", age_group))),
    age_broad = case_when(
      age_start < 15 ~ "Children (1–14)",
  ...
  summarise(pop = sum(population, na.rm = TRUE), .groups = "drop") %>%
  mutate(
    share_of_global_population = pop / sum(pop),
    share_pct = round(100 * share_of_global_population, 1)
  )

print(global_age_distribution)
#If we switch a 25% high-burden threshold for single age and sex strata, 3.1% of the global population resides in strata exceeding this level. Of this high-burden population, 71% live in Sub-Saharan Africa and 13.7% in Latin America and the Caribbean. The burden is distributed across age groups, with 36.2% occurring among adults aged 15–49 and 35.9% among children aged 1–14. Notably, 86% of individuals living in ≥25% strata are female, underscoring the disproportionate burden borne by women in the most affected populations (see Supplementary Table 1 for full country–age–sex distributions).




# ============================================================
# Figure 1 replacement — Energy-adequate assumption (2 maps)
#   Uses prot2_central (exact_kcal / kcal_consumed_optimal/EER)
#   Panels:
#     (a) Mean protein intake (g/day)
#     (b) Protein inadequacy (EAR) prevalence
# ============================================================

stopifnot(exists("prot2_central"))
stopifnot(exists("world"))
stopifnot(exists("normalize_iso3"))
stopifnot(exists("common_coord_sf"))
stopifnot(exists("tight_map_theme"))

country_exact <- prot2_central %>%
  group_by(iso3) %>%
  summarise(
    mean_protein_g_exact = weighted.mean(protein_grams_exact,    w = population, na.rm = TRUE),
    mean_pct_EAR_exact   = weighted.mean(prot_mean_pct_EAR_exact, w = population, na.rm = TRUE),
...
map_df_exact <- world %>%
  left_join(country_exact, by = "parent_iso3")
...
strong_map_theme <- function() {
  theme(
    plot.title = element_text(
      size = 20,          # bigger
      face = "bold",
      margin = margin(b = 8),
      hjust = 0.5         # centered (looks better for maps)
    ),
...
# Panel a: mean absolute protein, magma like Fig 3d, legend bottom, start at 0
p_opt_mean_protein_g <- ggplot(map_df_exact) +
  geom_sf(aes(fill = mean_protein_g_exact), color = NA) +
  scale_fill_viridis_c(
    option = "magma",
    direction = -1,
    limits = c(0, max(map_df_exact$mean_protein_g_exact, na.rm = TRUE)),
    oob = scales::squish,
    na.value = na_gray,
    labels = label_number(accuracy = 1)
  ) +
...
p_opt_mean_protein_g

# # Panel b: EAR inadequacy prevalence, red ramp, legend bottom, start at 0
# p_opt_inad_EAR <- ggplot(map_df_exact) +
#   geom_sf(aes(fill = prot_EAR_exact), color = NA) +
#   scale_fill_gradientn(
#     colours  = c("#f0f0f0", "#fdbb84", "#e34a33"),
#     limits   = c(0, 0.30),     # set cap; adjust if needed
#     oob      = scales::squish,
#     labels   = percent_format(accuracy = 1),
#     na.value = na_gray
#   ) +
#   common_coord_sf() + theme(
#     plot.title.position = "panel",
#     plot.title = element_text(
#       size = 20,
#       face = "bold",
...
#   ) +
#   labs(
#     title = "Protein inadequacy under energy-adequate assumption",
#     fill  = "Prevalence"
#   ) +
#   tight_map_theme() +
#   theme(
...
# 
# 
# p_opt_mean_protein_g_strong <- p_opt_mean_protein_g +
#   strong_map_theme()
# 
# ggsave(
#   "./output/Figure1_panel_a_mean_protein_g_day_energy_adequate.png",
#   p_opt_mean_protein_g_strong,
#   width = 12,
#   height = 6.6,
#   dpi = 450,
#   bg = "white"
...
# 
# 
# p_opt_inad_EAR_strong <- p_opt_inad_EAR +
#   strong_map_theme()
# 
# ggsave(
#   "./output/Figure1_panel_b_protein_inadequacy_energy_adequate.png",
#   p_opt_inad_EAR_strong,
#   width = 12,
#   height = 6.6,
#   dpi = 450,
#   bg = "white"
...
# Panel b: mean protein intake relative to EAR (deterministic)
# Positive values = mean intake above EAR
# Negative values = mean intake below EAR

country_exact_margin <- prot2_central %>%
  group_by(iso3) %>%
  summarise(
    mean_margin_EAR_g = weighted.mean(
      protein_grams_exact - ear_mean_g_day,
      w = population,
      na.rm = TRUE
    ),
    .groups = "drop"
...
map_df_exact_margin <- world %>%
  left_join(country_exact_margin, by = "parent_iso3")

margin_lim <- max(abs(map_df_exact_margin$mean_margin_EAR_g), na.rm = TRUE)

p_opt_margin_EAR <- ggplot(map_df_exact_margin) +
  geom_sf(aes(fill = mean_margin_EAR_g), color = NA) +
  scale_fill_gradient2(
    low = "#b2182b",
    mid = "grey97",
    high = "#2166ac",
    midpoint = 0,
    limits = c(-margin_lim, margin_lim),
    oob = scales::squish,
    na.value = na_gray,
    labels = label_number(accuracy = 1)
  ) +
  common_coord_sf() +
  labs(
    title = "Mean protein intake relative to the EAR under energy-adequate assumption",
    fill  = "g/day\nvs EAR"
  ) +
  tight_map_theme() +
  theme(
...
p_opt_margin_EAR_strong <- p_opt_margin_EAR +
  strong_map_theme()

ggsave(
  "./output/Figure1_panel_b_mean_protein_minus_EAR_energy_adequate.png",
  p_opt_margin_EAR_strong,
  width = 12,
  height = 6.6,
  dpi = 450,
  bg = "white"
...
# ============================================================
# Figure 1 — 1×2 GRID (TWO MAPS) + SAVE
#   Panels:
#     (a) p_opt_mean_pct_EAR
#     (b) p_opt_inad_EAR
# Output:
#   ./output/Figure1_2panel_energy_adequate_maps.png
# ============================================================

# library(patchwork)
# 
# # Optional: consistent title style + panel tags
# pA <- p_opt_mean_protein_g +
#   labs(tag = "a") +
...
#   theme(plot.title = element_text(size = 13, face = "bold"))
# 
# # 1×2 layout (like Fig 3 styling, just fewer panels)
# fig1_2panel <- (pA | pB) +
#   plot_layout(widths = c(1, 1)) +
#   plot_annotation(
#     theme = theme(
...
# # Save
# ggsave(
#   filename = "./output/Figure1_2panel_energy_adequate_maps.png",
#   plot = fig1_2panel,
#   width = 14.5,    # ~2 panels side-by-side
#   height = 6.6,    # map-friendly height
#   dpi = 450,
#   units = "in",
#   bg = "white",
#   limitsize = FALSE
...
prot2_central <- dat %>%
  ...
  mutate(
    ...
    # ---- Mean-only adequacy (no distribution) ----
    prot_mean_pct_EAR_exact    = 100 * (protein_grams_exact / ear_mean_g_day),
    prot_mean_pct_RDA_exact    = 100 * (protein_grams_exact / rda_mean_g_day),   # NEW
    prot_mean_pct_OPT_exact    = 100 * (protein_grams_exact / opt_mean_g_day),
    prot_mean_pct_EAR_exact_QA = 100 * (protein_grams_exact / ear_mean_g_day_adj),
    prot_inad_EAR_exact = calculate_inadequacy(
      mean_intake       = protein_grams_exact,
...
# Reordered so that:
#   (a) Mean protein intake (g/day)
#   (b) Protein inadequacy (EAR)
#   (c) Protein inadequacy relative to energy inadequacy
#   (d) Protein from animal-source foods / protein quality proxy
# ============================================================

library(patchwork)

# ---- Add panel tags + make titles consistent size ----
pA <- p_map_protein_g + labs(tag = "a") + theme(plot.title = element_text(size = 13, face = "bold"))
pB <- p_map_EAR       + labs(tag = "b") + theme(plot.title = element_text(size = 13, face = "bold"))
pC <- p_map_ratio3    + labs(tag = "c") + theme(plot.title = element_text(size = 13, face = "bold"))
pD <- p_map_quality   + labs(tag = "d") + theme(plot.title = element_text(size = 13, face = "bold"))
...
pA <- p_fig2 +
  coord_cartesian(ylim = c(0, ymax_common_fig2)) +
  labs(
    title = "Protein inadequacy by region",
    tag = "a"
  ) +
  theme(
    plot.title = element_text(size = 13, face = "bold")
...
# ==============================================================
# High-burden strata: protein inadequacy (EAR) >= 20%
# Medium ("realistic") calorie scenario
# ==============================================================
...
cat("\nGlobal share of population in strata >=20%:",
    round(100*global_share_pop_high, 2), "%\n")
print(share_pop_high_by_region, n = nrow(share_pop_high_by_region))
print(share_pop_high_by_sex, n = nrow(share_pop_high_by_sex))
...
# ==============================================================
# Age distribution of population living in >=20% strata
# ==============================================================
...
print(global_age_distribution)
#If we switch a 20% high-burden threshold for single age and sex strata, 3.1% of the global population resides in strata exceeding this level. Of this high-burden population, 71% live in Sub-Saharan Africa and 13.7% in Latin America and the Caribbean. The burden is distributed across age groups, with 36.2% occurring among adults aged 15–49 and 35.9% among children aged 1–14. Notably, 86% of individuals living in ≥25% strata are female, underscoring the disproportionate burden borne by women in the most affected populations (see Supplementary Table 1 for full country–age–sex distributions).
...
# Panel a: mean percentage protein (EAR), magma like Fig 3d, legend bottom, start at 0
p_opt_mean_pct_EAR <- ggplot(map_df_exact) +
  geom_sf(aes(fill = mean_pct_EAR_exact), color = NA) +
  scale_fill_viridis_c(
    option = "magma",
    direction = -1,
    limits = c(0, max(map_df_exact$mean_pct_EAR_exact, na.rm = TRUE)),
    oob = scales::squish,
    na.value = na_gray,
    labels = label_number(accuracy = 1)
  ) +
...
# 
# 
# p_opt_mean_pct_EAR_strong <- p_opt_mean_pct_EAR +
#   strong_map_theme()
# 
# ggsave(
#   "./output/Figure1_panel_a_mean_pct_EAR_energy_adequate.png",
#   p_opt_mean_pct_EAR_strong,
#   width = 12,
#   height = 6.6,
#   dpi = 450,
#   bg = "white"
...
# ============================================================
# Figure 1 panel b replacement — country-level mean protein
# relative to RDA, by World Bank region and sex
# Deterministic only: no intake distributions
# Uses prot2_central
# ============================================================

stopifnot(exists("prot2_central"))

# 1) Country-level sex-specific means under the energy-adequate assumption
fig1b_region_sex <- prot2_central %>%
  group_by(iso3, region_WB, sex) %>%
  summarise(
    country_mean_pct_RDA = weighted.mean(
      prot_mean_pct_RDA_exact,
      w = population,
      na.rm = TRUE
    ),
    .groups = "drop"
  ...
  mutate(
    sex = case_when(
      sex %in% c("Female", "female", "F", "Females") ~ "Female",
      sex %in% c("Male", "male", "M", "Males") ~ "Male",
      TRUE ~ as.character(sex)
    )
  )

# Optional: add a "Global" category so the first pair is world-wide
fig1b_global <- fig1b_region_sex %>%
  mutate(region_WB = "Global")

fig1b_plot_df <- bind_rows(fig1b_global, fig1b_region_sex)

# 2) Order regions by pooled median across both sexes (Global first)
region_order <- fig1b_region_sex %>%
  group_by(region_WB) %>%
  summarise(
    region_median = median(country_mean_pct_RDA, na.rm = TRUE),
    .groups = "drop"
  ) %>%
...
fig1b_plot_df <- fig1b_plot_df %>%
  mutate(
    region_WB = factor(region_WB, levels = c("Global", region_order)),
    sex = factor(sex, levels = c("Female", "Male"))
  )

# 3) Paired boxplots
p_opt_box_region_sex <- ggplot(
  fig1b_plot_df,
  aes(x = region_WB, y = country_mean_pct_RDA, fill = sex)
) +
  geom_boxplot(
...
# Save panel b alone
ggsave(
  filename = "./output/Figure1_panel_b_region_sex_boxplots_pctRDA_energy_adequate.png",
  plot = p_opt_box_region_sex,
  width = 10.5,
  height = 5.8,
  dpi = 450,
  units = "in",
...
# ============================================================
# Countries where protein inadequacy (EAR) > caloric inadequacy (MDER)
# Medium scenario (realistic_calories_medium)
# ============================================================

stopifnot(exists("scen3_long"))

# 1) Country-level prevalences in medium scenario
country_gap_tbl <- scen3_long %>%
  filter(calorie_scenario == "medium") %>%
  group_by(iso3) %>%
  summarise(
    protein_inad_EAR = weighted.mean(prot_inad_EAR, w = population, na.rm = TRUE),
    calorie_inad_MDER = weighted.mean(cal_inad,     w = population, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
    country = countrycode(iso3, "iso3c", "country.name"),
    region_WB = countrycode(iso3, "iso3c", "region"),
    gap_pp = 100 * (protein_inad_EAR - calorie_inad_MDER),
    ratio = protein_inad_EAR / calorie_inad_MDER
  )

# 2) Main list: protein > calories (no extra threshold)
countries_protein_gt_cal <- country_gap_tbl %>%
  filter(
    is.finite(protein_inad_EAR),
    is.finite(calorie_inad_MDER),
    protein_inad_EAR > calorie_inad_MDER
  ) %>%
  arrange(desc(gap_pp))

# 3) “Strict” list: avoid near-zero calorie inadequacy (ratio becomes unstable)
countries_protein_gt_cal_strict <- country_gap_tbl %>%
  filter(
    is.finite(protein_inad_EAR),
    is.finite(calorie_inad_MDER),
    calorie_inad_MDER >= 0.005,                 # >= 0.5% energy inadequacy
    protein_inad_EAR > calorie_inad_MDER
  ) %>%
  arrange(desc(gap_pp))

# --- Quick sanity summaries ---
cat("\nHow many countries have protein inadequacy > calorie inadequacy?\n")
cat("  Any (no threshold):", nrow(countries_protein_gt_cal), "\n")
cat("  Strict (calorie>=0.5%):", nrow(countries_protein_gt_cal_strict), "\n")

cat("\nRegion breakdown (STRICT list):\n")
print(
  countries_protein_gt_cal_strict %>%
    count(region_WB, sort = TRUE),
  n = 50
)

cat("\nTop 25 countries by (protein - calorie) gap, percentage points (STRICT):\n")
print(
  countries_protein_gt_cal_strict %>%
    transmute(
      iso3, country, region_WB,
      protein_inad_EAR_pct = 100 * protein_inad_EAR,
      calorie_inad_MDER_pct = 100 * calorie_inad_MDER,
      gap_pp,
      ratio
    ) %>%
    head(25),
...
# 4) Export (nice for supplement tables)
write_csv(
  countries_protein_gt_cal_strict %>%
    transmute(
      iso3, country, region_WB,
      protein_inad_EAR = protein_inad_EAR,
      calorie_inad_MDER = calorie_inad_MDER,
      protein_inad_EAR_pct = 100 * protein_inad_EAR,
      calorie_inad_MDER_pct = 100 * calorie_inad_MDER,
      gap_pp,
      ratio
    ),
  "./output/countries_protein_inad_gt_cal_inad_medium_STRICT.csv"
)

write_csv(
  countries_protein_gt_cal %>%
    transmute(
      iso3, country, region_WB,
      protein_inad_EAR = protein_inad_EAR,
      calorie_inad_MDER = calorie_inad_MDER,
  ...
  "./output/countries_protein_inad_gt_cal_inad_medium_ANY.csv"
)
...
# Country-level mean protein intake (g/day) under energy-adequate counterfactual
fig1a_g_df <- prot2_central %>%
  group_by(iso3, region_WB, sex) %>%
  summarise(
    country_mean_protein_g = weighted.mean(
      protein_grams_exact,
      w = population,
      na.rm = TRUE
    ),
...
cat("\n=== GLOBAL SUMMARY BY SEX (g/day) ===\n")
global_g_stats <- fig1a_g_df %>%
  group_by(sex) %>%
  summarise(
    n_countries = n(),
    median_g_day = median(country_mean_protein_g, na.rm = TRUE),
    p25 = quantile(country_mean_protein_g, 0.25, na.rm = TRUE),
    p75 = quantile(country_mean_protein_g, 0.75, na.rm = TRUE),
    min = min(country_mean_protein_g, na.rm = TRUE),
    max = max(country_mean_protein_g, na.rm = TRUE),
    .groups = "drop"
  )
print(global_g_stats, n = nrow(global_g_stats), width = Inf)

cat("\n=== REGION x SEX SUMMARY (g/day) ===\n")
region_g_stats <- fig1a_g_df %>%
  filter(!is.na(region_WB), !is.na(sex)) %>%
  group_by(region_WB, sex) %>%
  summarise(
    n_countries = n(),
  ...
  arrange(region_WB, sex)
print(region_g_stats, n = nrow(region_g_stats), width = Inf)

cat("\n=== LOWEST 10 COUNTRY-SEX OBSERVATIONS (g/day) ===\n")
lowest_g <- fig1a_g_df %>%
  arrange(country_mean_protein_g) %>%
  select(country, iso3, region_WB, sex, country_mean_protein_g) %>%
  head(10)
print(lowest_g, n = nrow(lowest_g), width = Inf)




# Global + sex-specific population-weighted mean protein intake (% of RDA)
headline_rda <- bind_rows(
  prot2_central %>%
    summarise(
      group = "Global",
      mean_pct_RDA = weighted.mean(prot_mean_pct_RDA_exact, w = population, na.rm = TRUE)
    ),
  prot2_central %>%
    group_by(sex) %>%
    summarise(
    ...
    mutate(group = sex) %>%
    select(group, mean_pct_RDA)
) %>%
  mutate(
    mean_pct_RDA = round(mean_pct_RDA, 1)
  )
