...
p_opt_mean_pct_EAR <- ggplot(map_df_exact) +
  ...
  common_coord_sf() +
  labs(
    title = "Mean protein intake (% of EAR), energy-adequate assumption",
    fill  = "% of EAR"
  ) +
  tight_map_theme() +
  theme(
...
pA <- p_opt_mean_pct_EAR + labs(tag = "a") + theme(plot.title = element_text(size = 13, face = "bold"))
pB <- p_opt_inad_EAR     + labs(tag = "b") + theme(plot.title = element_text(size = 13, face = "bold"))

fig1_replacement_maps <- (pA | pB) +
  plot_layout(widths = c(1, 1)) +
  plot_annotation(theme = theme(plot.margin = margin(6, 6, 6, 6)))

ggsave(
  filename = "./output/Figure1_energy_adequate_maps_ab.png",
  plot = fig1_replacement_maps,
  width = 14.5,
  height = 6.2,
  dpi = 450,
  units = "in",
